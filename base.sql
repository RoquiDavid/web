create table user (
  ...
  id int primry key not null auto_increment);

create table session(
  key_session varchar(32) not null primary key,
  id_user int not null,
  time timestamp not null,
  constraint fk_session foreigh key (id_user) references user(id) on delete cascade);
...
create table block(
  ...
  idCurrentUser int,
  idBlockedUser int,
  constraint pk_block_primary key (idCurrentUser, idBlockedUser),
